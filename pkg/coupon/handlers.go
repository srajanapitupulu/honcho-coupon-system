...
func SetupRouter(database *sql.DB) *gin.Engine {
	...
	r.POST("/api/coupons", func(c *gin.Context) { createCoupon(c, database) })
	r.POST("/api/coupons/claim", func(c *gin.Context) { claimCoupon(c, database) })
	r.GET("/api/coupons/:name", func(c *gin.Context) { getCouponDetails(c, database) })
...
func claimCoupon(c *gin.Context, db *sql.DB) {
...
func createCoupon(c *gin.Context, db *sql.DB) {
	...
	if err != nil {
		if pgErr, ok := err.(*pgconn.PgError); ok {
            if pgErr.Code == "23505" { 
				// Check for unique_violation error code if coupon name already exists
				// 23505 is the Postgres code for unique_violation
                c.JSON(http.StatusConflict, gin.H{"error": "Coupon with this name already exists"})
            }
        } else {
...
func getCouponDetails(c *gin.Context, db *sql.DB) {
